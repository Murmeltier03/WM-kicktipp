const FINAL_POINTS = [
  ...
  { name: "Robin", points: 4 },
  { name: "Denis", points: 0 },
  { name: "Yannic", points: 2 },
  { name: "Anka", points: 2 },
  { name: "Yannick", points: 2 },
  { name: "Marius", points: 2 },
  { name: "Moritz", points: 0 },
] as const;

const EXPECTED_RESTORE_ROWS = FINAL_POINTS.length;
const EXPECTED_RESTORE_TOTAL = FINAL_POINTS.reduce<number>((total, player) => total + player.points, 0);
...
async function loadRestoreContext(supabase: Awaited<ReturnType<typeof adminClient>>) {
  const expectedNames = FINAL_POINTS.map((player) => player.name);
  ...
  if ((players ?? []).length !== FINAL_POINTS.length || playerIdByName.size !== FINAL_POINTS.length) {
    throw new Error(`Expected 9 unique matching WM players, found ${players?.length ?? 0}`);
  }

  const expectedRows = FINAL_POINTS.map((player) => ({
    tournament_slug: "wm-2026",
    player_id: playerIdByName.get(player.name)!,
    kicktipp_matchday: 15,
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { error: tournamentError } = await supabase
      .from("tournaments")
      .update({ player_count: FINAL_POINTS.length, updated_at: updatedAt })
      .eq("slug", "wm-2026");
